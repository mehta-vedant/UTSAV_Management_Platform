import { Prisma } from "@prisma/client";
import { Calendar } from "lucide-react";
import { Card, CardHeader, CardTitle, CardContent } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";

interface DonationListProps {
    donations: {
        id: string;
        donorName: string;
        amount: Prisma.Decimal;
        category: string | null;
        paymentMode: string | null;
        date: Date;
    }[];
}

export default function DonationList({ donations }: DonationListProps) {
    return (
        <Card className="border-none shadow-sm rounded-3xl overflow-hidden bg-white h-full flex flex-col">
            <CardHeader className="pb-4">
                <CardTitle className="text-xl font-black text-slate-900 tracking-tight">Recent Donations</CardTitle>
            </CardHeader>

            <CardContent className="flex-1 px-6 pb-6">
                <div className="space-y-4">
                    {donations.map((d) => (
                        <div key={d.id} className="flex items-center justify-between gap-3 p-4 rounded-2xl border border-slate-100 bg-slate-50/40">
                            <div className="flex items-center gap-3 min-w-0">
                                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-saffron-50 text-sm font-black text-saffron-600">
                                    {d.donorName.charAt(0)}
                                </div>
                                <div className="min-w-0">
                                    <h4 className="truncate text-sm font-bold text-slate-900">{d.donorName}</h4>
                                    <div className="mt-1 flex flex-wrap items-center gap-1.5">
                                        <Badge variant="secondary" className="bg-white text-[9px] font-black uppercase text-slate-500 border-slate-100">
                                            {d.category || "General"}
                                        </Badge>
                                        {d.paymentMode && (
                                            <Badge variant="secondary" className="bg-white text-[9px] font-black uppercase text-blue-500 border-blue-100">
                                                {d.paymentMode.replace("_", " ")}
                                            </Badge>
                                        )}
                                    </div>
                                </div>
                            </div>
                            <div className="text-right shrink-0">
                                <p className="text-sm font-black text-slate-900 leading-none">₹{Number(d.amount).toLocaleString("en-IN")}</p>
                                <span className="mt-1 block text-[10px] font-bold text-slate-400">
                                    <Calendar className="mr-1 inline h-2.5 w-2.5" />
                                    {new Date(d.date).toLocaleDateString("en-IN", { day: 'numeric', month: 'short' })}
                                </span>
                            </div>
                        </div>
                    ))}

                    {donations.length === 0 && (
                        <div className="py-16 text-center text-sm font-bold text-slate-400 uppercase tracking-tight bg-slate-50 rounded-3xl border border-dashed border-slate-200">
                            No donations yet
                        </div>
                    )}
                </div>
            </CardContent>
        </Card>
    );
}
