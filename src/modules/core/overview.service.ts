...
import { getOrganizationFinancialOverview, getRecentActivity, OrganizationFinancialOverviewResult } from "../finance/financial.service";
import { OrganizationRole, Prisma } from "@prisma/client";
import { getOrganizationSummary, OrganizationFinancialSummary } from "../festival/organization-financial.service";
...
export interface DashboardOverview {
    ...
    financials: OrganizationFinancialOverviewResult;
    ...
    recentActivity: Array<{ id: string; type: "DONATION" | "EXPENSE" | "MEMBER"; title: string; amount?: number; date: Date; status?: string; identity?: string }>;
    stats: {
        volunteers: {
            total: number;
            unassigned: number;
...
export async function getOrganizationOverview(organizationId: string): Promise<DashboardOverview> {
    ...
    let financials: OrganizationFinancialOverviewResult = {
        totalDonations: 0,
    ...
    if (canViewFinancials) {
        [financials, eventFinancials] = await Promise.all([
            getOrganizationFinancialOverview(organizationId),
            getOrganizationSummary(organizationId)
        ]);
    }

    const hasTreasurer = memberStats.some((m) => m.role === OrganizationRole.TREASURER);

    const rolesCount = memberStats.reduce((acc: Record<string, number>, curr) => {
    ...
    return {
        ...
        stats: {
            ...
            members: {
                total: memberStats.filter((m) => m.role !== OrganizationRole.VOLUNTEER).length,
                byRole: rolesCount
            },
            events: {
                total: eventStats.length,
                upcoming: eventStats.filter((e) => e.startTime > new Date()).length
            }
        }
    };
}
