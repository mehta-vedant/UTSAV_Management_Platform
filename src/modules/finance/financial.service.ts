...
import { DonationCategory, ExpenseStatus, Prisma } from "@prisma/client";

export interface OrganizationFinancialOverviewResult {
...
export async function getOrganizationFinancialOverview(organizationId: string): Promise<OrganizationFinancialOverviewResult> {
    ...
    const [funds, sponsorships, expenses, pendingExpenses, bhogItems, Organization] = await Promise.all([
        ...
        tenantPrisma.donation.aggregate({
            where: {
                isArchived: false,
                category: { not: DonationCategory.SPONSORSHIP }
            },
            _sum: { amount: true },
            _count: { _all: true },
        }),
        ...
        tenantPrisma.donation.aggregate({
            where: {
                isArchived: false,
                category: DonationCategory.SPONSORSHIP
            },
            _sum: { amount: true },
            _count: { _all: true },
        }),
    ...
    // Total Collection = Opening Balance + Member Donations + Sponsorships
    const totalCollection = openingBalance.plus(memberDonations).plus(totalSponsorships);
